									...
									<div class="card-header align-items-center py-5 gap-2 gap-md-5">
										...
										<div class="card-title">
											...
											<div class="d-flex align-items-center position-relative my-1">
												...
												<!--end::Svg Icon-->
												<input  v-model="search" type="text" data-kt-ecommerce-product-filter="search" class="form-control form-control-solid w-250px ps-14" placeholder="Search for user Rate" />
											</div>
											<!--end::Search-->
										</div>
										<!--end::Card title-->
										...
										<table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_products_table">
											...
											<thead>
												...
												<tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
													...
													<th class="text-end min-w-70px">Rate</th>
...
C/O https://placeholder.com/ ');"></span>
															...
															<div class="ms-5">
																<!--begin::Title-->
																<a href="#" class="text-gray-800 text-hover-primary fs-5 fw-bolder" data-kt-ecommerce-product-filter="product_name">{{rate.user.name!=null ?  rate.user.name  : 'Default user'}}</a>
																<!--end::Title-->
															</div>
														</div>
													</td>
													...
													<td class="text-end pe-0" data-order="rating-4">
														<div class="rating justify-content-end" style="display:flex; justify-content:space-between">
													      <font-awesome-icon v-for="star in rate.rate" :key="star" icon="fas fa-star" />
														</div>
													</td>
													<!--end::Rating-->
													<!--begin::Status=-->
													<td class="text-end pe-0" data-order="Published">
														<!--begin::Badges-->
														<div class="">{{rate.product.name}}</div>
														<!--end::Badges-->
													</td>
													<!--end::Status=-->
													<!--begin::Action=-->
													...
													<td class="text-end">
															...
															<a @click.prevent="deleteRate(rate.id)" href="#" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm">
                        ...
                        <ul class="pagination">
                            <li class="paginate_button page-item previous disabled" id="kt_table_users_previous"
                                ...
                                <a href="javascript:void(0)" aria-controls="kt_table_users" data-dt-idx="0" tabindex="0"
                                    class="page-link" v-on:click.prevent="getRates(paginate.current_page - 1)"><i
                                        class="previous"></i></a>
                            </li>
                            <li class="paginate_button page-item" v-for="page in pages" :key="page"
                                ...
                                <a href="javascript:void(0)" aria-controls="kt_table_users" class="page-link"
                                    v-on:click.prevent="getRates(page)">{{ page }}</a>
                            </li>
...
export default {
    ...
    methods:{
         getRates(page=1)
        {
            axios.get('/api/admin/rates?page='+page+"&&paginate="+this.perpage+"&&search="+this.search)
            .then(response=>{
        ...
        deleteRate(id)
        {
            axios.post('/api/admin/rates/delete/'+id)
            .then(response=>{
            ...
            .catch(errors=>{
                 this.$Message['error']({
                    background: true,
                    content: 'Rate Not Found Server Error! '
                });
            })
        }
    },
